...
window.addEventListener("DOMContentLoaded", (event) => {
  ...
  const btnGalleryAll1 = document.querySelectorAll(".popup-large-1");
  const galleryPopup1 = document.querySelector("#bg-gallery-popup-1");
  const galleryClose1 = document.querySelector("#bg-gallery-back-1");
  ...
  const bAccordion = document.querySelectorAll(".bAccordion");
  ...
  // Popup1 - Large
  for (let i = 0; i < btnGalleryAll1.length; i++) {
    btnGalleryAll1[i].addEventListener("click", function () {
      galleryPopup1.classList.remove("hidden");
    });
  }

  function closePopup1() {
    galleryPopup1?.classList.add("hidden");
    body.classList?.remove("no-overflow-y");
  }

  const animationEnd1 = function () {
    const checkPoputActive = !galleryPopup1?.classList.contains("hidden");
    if (checkPoputActive) body.classList.add("no-overflow-y");
  };

  if (galleryClose1) {
    galleryClose1.addEventListener("click", closePopup1);
    galleryPopup1.addEventListener("transitionend", animationEnd1);
    galleryPopup1.addEventListener("webkitTransitionEnd", animationEnd1);
  }

  document.addEventListener("keydown", function (e) {
    if (e.key === "Escape" && !galleryPopup1?.classList.contains("hidden"))
      closePopup1();
  });
  ...
  // Accordion
  for (let i = 0; i < bAccordion.length; i++) {
    bAccordion[i].addEventListener("click", function () {
      bAccordion[i].classList.toggle("collapsed");
    });
  }

  // // Packages Collapse
  // for (let i = 0; i < bgCollapses.length; i++) {
  //   bgCollapses[i].addEventListener("click", function () {
  //     console.log("Alwin");
  //     bgCollapses[i].classList.toggle("collapsed");
  //   });
  // }
