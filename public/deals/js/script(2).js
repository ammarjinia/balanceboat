...
window.addEventListener("DOMContentLoaded", (event) => {
  ...
  for (let i = 0; i < btnGalleryAll.length; i++) {
    btnGalleryAll[i].addEventListener("click", function () {
       jQuery("#bg-gallery-popup[data-rel='"+jQuery(this).attr("data-rel")+"']").removeClass("hidden");
       // galleryPopup.classList.remove("hidden");
    });
  }
  
  jQuery(".gallery-back-btn").on("click", function(){
    jQuery(".gallery-popup").addClass("hidden");
  });
  
  jQuery(document).on("keydown", function(e){
      if (e.key === "Escape" && (jQuery(".gallery-popup").not(".hidden").length > 0)) {
        jQuery(".gallery-popup").addClass("hidden");
      }
  });
