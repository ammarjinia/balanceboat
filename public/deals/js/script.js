...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  const btnGalleryAll = document.querySelectorAll(
    "#bg-gallery-all, .main .bg-listing-gallery-items, .popup-large"
  );
  ...
  for (let i = 0; i < buttonsOpen.length; i++) {
    ...
    const closeModal = function () {
        ...
        if ($("#popup-call-back").length > 0) {
            $("#frmRequestCall").validate().resetForm();
            $("#frmRequestCall .required").removeClass("error");
            $("#popup-call-back .alert").remove();
        }
...
////////////// Custom JS /////////
$(document).on("click", ".exp-popup-large", function() {
    $(".gallery-popup[data-rel="+$(this).attr("data-rel")+"]").removeClass("hidden");
});
$(document).on("click", ".gallery-back-btn", function() {
