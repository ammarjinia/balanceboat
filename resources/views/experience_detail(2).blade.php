{{--
    ...
    Full Booking Layout Concept"). Separate view from experience_detail.blade.php by design — reachable
    via the existing ExperienceController@index ?test=1 toggle, so the live page is untouched.

    Reuses real data exactly as prepared by ExperienceController@index (same $data keys as the live page)
    and reuses four existing partials unchanged for their PHP logic/JS hooks — only their CSS is
    re-skinned here, under the .rd-page scope, to match the artifact:
    ...
    here to avoid colliding with the site's Bootstrap/global CSS), wired to the same real fields the live
...
<div class="rd-page">
  ...
  {{-- Mobile persistent booking bar + drawer — real partial, re-skinned via CSS above.
       Deliberately kept INSIDE .rd-page (matches experience_detail.blade.php's .xd-page structure) —
       every .rd-page .xd-mobile-* rule above depends on this ancestor to match at all. --}}
...
<script type="text/javascript">
    ...
    // mark (see the live experience_detail.blade.php fix): lazy-loaded images can still be resizing
    // the page shortly after 'load', so a single early calculation can under/over-shoot.
    ...
    // behavior, also present on the live experience_detail.blade.php page, not something new here.
    // A hardcoded `top: 80px` on .rd-page-nav would then leave a stale 80px gap once the header is
    // gone, with page content bleeding up through it. Re-measuring the header on every scroll/resize
    // keeps the nav pinned directly under wherever the header currently ends (or top:0 once it's gone).
